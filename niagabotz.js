...
async function getList(kategori, brand, type) {
	...
	`- Untuk Order, Ketik: *topup <kode>|<tujuan>*\n\n` +
	`- Contoh: *topup ML3|2400388271127*\n\n` +
	`─────────────────────\n\n`;
	filteredProducts.forEach((item) => {
		...
		productListText += `🛍️ *${item.product_name}*\n`;
	...
	// await kris.sendMessage(from, {text: `Untuk Order Ketik : topup <kode>|<tujuan> \nContoh topup ML3|2400388271127`})
}
...
async function getWallet(kategori, brand, tipe) {
    ...
    const calculatePriceWithProfit = (item) => {
        let profitNominal = profitConfig[item.brand.toUpperCase()] || 0; // Default ke 0 jika tidak ditemukan profit
    ...
    // Menyusun daftar produk dalam format teks biasa
    let productListText = `✨ *Daftar Produk ${brand} - ${tipe}:* ✨\n\n` +
    `- Untuk Order, Ketik: *topup <kode>|<tujuan>*\n\n` +
    `- Contoh: *topup PLN|2400388271127*\n\n` +
    `──────────────────────\n\n`;

    filteredProducts.forEach((item) => {
    	const status = item.seller_product_status ? "✅ Tersedia" : "⛔ Gangguan";
        const formattedPrice = formatMoney(calculatePriceWithProfit(item)); // Menggunakan harga dengan profit
        productListText += `🛍️ *${item.product_name}* \n`;
        productListText += `🔖 Status: ${status}\n`;
        productListText += `💰 Harga: ${formattedPrice}\n`;
        productListText += `🔑 Kode: ${item.buyer_sku_code}\n\n`;
...
if (command) {
    ...
    case "pay":
    {
    	...
    	if (text.toLowerCase() == "h") {
    		let depositInfo = "`INFORMASI PAYMENTS`\n\n" +
    		`> DANA  : 085812486745 - MUJX\n` +
    		`> GOPAY : 085812486745\n` +
    		`> BNC   : 085812486745\n\n` +
    		"`A.N HAMDAN MUBAROK`\n\n" +
...
case "asli": 
{
    ...
    if (cek("id", m.sender) == null) {
    	daftarr();
    	return reply("_Kamu telah terdaftar, silakan ketik *menu* untuk melihat daftar layanan._");
    }

    // Informasi pengguna dan bot
    const welcomeMessage = `🗣️ Halo Kak ${pushname}, jika menemukan bug, silakan lapor ke Owner.\n` +
    `⚝ Informasi Bot ⚝\n\n` +
    `≡ Nama Bot: ${namabot}\n` +
    `≡ Nama Anda: ${pushname}\n` +
    `≡ Nomor Owner: ${owner}\n` +
    ...
    // Menu dalam bentuk teks biasa
    const menuText = 
    `━━━━━━━━━━━━━━━━━━━━\n` +
    `*Menu Topup 🛒*\n` +
    `1. Top-Up Pulsa - Untuk melihat layanan top-up pulsa, ketik: .pulsa\n` +
    `2. Top-Up Kuota - Untuk melihat layanan top-up kuota, ketik: .kuota\n` +
    `3. Top-Up Games - Untuk melihat layanan top-up game, ketik: .topupgames\n` +
    `4. Top-Up E-Money - Untuk melihat layanan top-up e-money, ketik: .topupemoney\n` +
    `5. Top-Up Voucher - Untuk melihat layanan top-up voucher, ketik: .menuvoucher\n` +
    `6. Top-Up Token PLN - Untuk melihat layanan token PLN, ketik: .pln\n` +
    `\n` +
    `━━━━━━━━━━━━━━━━━━━━\n` +
    `*Menu SMM 🤖*\n` +
    `1. Layanan TikTok - Ketik: .tiktok\n` +
    `2. Layanan Instagram - Ketik: .instagram\n` +
    `3. Layanan YouTube - Ketik: .youtube\n` +
    `4. Layanan Facebook - Ketik: .facebook\n` +
    `\n` +
    `━━━━━━━━━━━━━━━━━━━━\n` +
    `*Menu Cek ID 🐱‍🚀*\n` +
    `1. Cek ID Games - Ketik: .cekgame\n` +
    `2. Cek Nama Bank - Ketik: .cekbank\n` +
    `3. Cek E-Wallet - Ketik: .cekwallet\n` +
    `\n` +
    `━━━━━━━━━━━━━━━━━━━━\n` +
    `*Menu Lainnya 🧾*\n` +
    `1. Menu Owner - Ketik: .ownermenu\n` +
    `2. Deposit Manual - Ketik: .manual\n` +
    `3. Deposit Otomatis - Ketik: .otomatis\n` +
    `4. List Transaksi - Ketik: .listtrx\n` +
    `5. Upgrade Role TopUp - Ketik: .upgraderole\n` +
    `6. Upgrade Role SMM - Ketik: .upgraderolesmm\n` +
    `7. List Deposit - Ketik: .listdeposit\n` +
    `8. Cek Saldo - Ketik: .ceksaldo\n` +
    `9. Batalkan Deposit - Ketik: .ndepo\n` +
    `\n` +
    `━━━━━━━━━━━━━━━━━━━━\n` +
    `Silakan ketik perintah sesuai dengan menu yang ingin Anda akses.`;




    // Mengirimkan pesan ke WhatsApp
    await kris.sendMessage(from, { text: welcomeMessage + menuText }, { quoted: m });
}
...
case "block":
{
  ...
  case "topupgames":
  {
  	...
  	let Anu = `🗣️ : *Hay Kak ${pushname} Jika Ada Bug Silahkan Lapor Ke Owner*\n` +
  	`⚝ *I N F O - B O T* ⚝\n\n` +
  	`≡ *NamaBot* : ${namabot}\n` +
  	`≡ *Nama* : ${pushname}\n` +
  	`≡ *Nomor Owner* : ${owner}\n` +
  	...
  	`1. Diamonds Mobile Legends - Ketik: \`.ml\`\n` +
  	`2. Diamonds Mobile Legends Global - Ketik: \`.mlg\`\n` +
  	`3. Diamonds Freefire - Ketik: \`.ff\`\n` +
  	`4. UC Pubg Mobile - Ketik: \`.pubg\`\n` +
  	`5. Tokens Honor OF Kings - Ketik: \`.hok\`\n` +
  	`6. Voucher Arena OF Valor - Ketik: \`.aov\`\n` +
  	`7. Points Valorant - Ketik: \`.valorant\`\n` +
  	`8. Garena Undawn - Ketik: \`.undawn\`\n` +
  	`9. The Ants Underground Kingdom - Ketik: \`.tac\`\n` +
  	`10. Call of Duty MOBILE - Ketik: \`.cod\`\n` +
  	`11. League of Legends Wild Rift - Ketik: \`.lol\`\n` +
  	`12. Metal Slug Awakening - Ketik: \`.metalslug\`\n` +
  	`13. Genshin Impact - Ketik: \`.genshin\`\n` +
  	`14. Diamonds 8 Ball Pool Coins - Ketik: \`.8polcoin\`\n` +
  	`15. Diamonds 8 Ball Pool Cash - Ketik: \`.8polcash\`\n` +
  	`16. Higgs Domino - Ketik: \`.higs\`\n` +
  	`17. FC Mobile - Ketik: \`.fcm\`\n\n` +
  	`*Topup Membership*\n\n` +
  	`1. Membership Mobile Legends - Ketik: \`.memberml\`\n` +
  	`2. Membership Freefire - Ketik: \`.memberff\`\n` +
  	`3. Membership Genshin Impact - Ketik: \`.membergenshin\`\n\n` +
  	`Silakan pilih topup games atau membership yang diinginkan dengan mengetik perintah yang sesuai.`;

  	await kris.sendMessage(from, { text: Anu }, { quoted: m });
  }
  break;
  ...
  case "pulsa": 
  {
  	...
  	if (cek("id", m.sender) == null) {
  		daftarr(); 
  		return reply(`_Silahkan ketik *menu* kembali_`);
  	}
  	...
  	`≡ *Saldo* : ${formatmoney(slo)}\n` +
  	`≡ *Role* : ${cek("role", m.sender)}\n\n` +
  	`*Berikut adalah daftar operator pulsa yang tersedia:*\n\n` +
  	`1. Pulsa Axis - Ketik: \`.menupulsaaxis\`\n` +
  	`2. Pulsa Telkomsel - Ketik: \`.menupulsatelkomsel\`\n` +
  	`3. Pulsa Indosat - Ketik: \`.menupulsaindosat\`\n` +
  	`4. Pulsa Smartfren - Ketik: \`.menupulsasmartfren\`\n` +
  	`5. Pulsa XL - Ketik: \`.menupulsaxl\`\n` +
  	`6. Pulsa Tri - Ketik: \`.menupulsatri\`\n` +
  	`7. Pulsa By.U - Ketik: \`.menupulsabyu\`\n\n` +
  	`Silakan pilih salah satu operator pulsa di atas dengan mengetik perintah yang sesuai.`;
  ...
  case "topupemoney":
  {
  	...
  	`≡ *Saldo* : ${formatmoney(slo)}\n` +
  	`≡ *Role* : ${cek("role", m.sender)}\n\n` +
  	`*Berikut adalah daftar layanan E-Money yang tersedia:*\n\n` +
  	`1. Layanan Dana - Ketik: .dana\n` +
  	`2. Layanan Ovo - Ketik: .ovo\n` +
  	`3. Layanan ShopeePay - Ketik: .shopeepay\n` +
  	`4. Layanan Gopay/Gojek - Ketik: .gopay\n\n` +
  	`Silakan pilih salah satu layanan E-Money di atas dengan mengetik perintah yang sesuai.`;
  ...
  case "menuvoucher":
  {
  	...
  	`≡ *Saldo* : ${formatmoney(slo)}\n` +
  	`≡ *Role* : ${cek("role", m.sender)}\n\n` +
  	`*Berikut adalah daftar layanan Voucher yang tersedia:*\n\n` +
  	`1. Google Play ID - Ketik: .vc-playid\n` +
  	`2. Google Play US - Ketik: .vc-tri\n` +
  	`3. Voucher GARENA - Ketik: .vc-garena\n` +
  	`4. Voucher Telkomsel - Ketik: .vc-telkomsel\n` +
  	`5. Voucher Indosat - Ketik: .vc-indosat\n` +
  	`6. Voucher Axis - Ketik: .v-axis\n` +
  	`7. Voucher Tri - Ketik: .vc-tri\n` +
  	`8. Voucher XL - Ketik: .vc-xl\n\n` +
  	`Silakan pilih layanan Voucher yang diinginkan dengan mengetik perintah yang sesuai.`;
  ...
  case "menupulsaindosat":
  {
  	...
  	let Anu = `🗣️ : *Hai Kak ${pushname}, jika ada bug silahkan lapor ke Owner*\n` +
  	`⚝ *INFORMASI BOT* ⚝\n\n` +
  	`≡ *Nama Bot* : ${namabot}\n` +
  	`≡ *Nama* : ${pushname}\n` +
  	`≡ *Nomor Owner* : ${owner}\n` +
  	`≡ *Saldo* : ${formatmoney(slo)}\n` +
  	`≡ *Role* : ${cek("role", m.sender)}\n\n` +
  	`*Berikut adalah daftar Pulsa Indosat yang tersedia:*\n\n` +
  	`1. Pulsa Normal - Ketik: .pulsaind\n` +
  	`2. Pulsa Transfer - Ketik: .pulsaind-transfer\n` +
  	`3. Pulsa Mix - Ketik: .pulsaind-mix\n` +
  	`4. Pulsa Gift - Ketik: .pulsaind-gift\n` +
  	`5. Pulsa Reguler Combo - Ketik: .pulsaind-reguler-combo\n` +
  	`6. Pulsa Combo Data - Ketik: .pulsaind-combo-data\n\n` +
  	`Silakan pilih salah satu layanan di atas dengan mengetik perintah yang sesuai.`;
  ...
  case "menupulsatelkomsel":
  {
  	...
  	`≡ *Saldo* : ${formatmoney(slo)}\n` +
  	`≡ *Role* : ${cek("role", m.sender)}\n\n` +
  	`*Berikut adalah daftar Pulsa Telkomsel yang tersedia:*\n\n` +
  	`1. Pulsa Normal - Ketik: .pulsatsel-umum\n` +
  	`2. Pulsa Transfer - Ketik: .pulsatsel-transfer\n\n` +
  	`Silakan pilih salah satu layanan di atas dengan mengetik perintah yang sesuai.`;
  ...
  case "menupulsaaxis":
  {
  	...
  	`≡ *Saldo* : ${formatmoney(slo)}\n` +
  	`≡ *Role* : ${cek("role", m.sender)}\n\n` +
  	`*Berikut adalah daftar Pulsa Axis yang tersedia:*\n\n` +
  	`1. Pulsa Normal - Ketik: .pulsaaxis-umum\n` +
  	`2. Pulsa Transfer - Ketik: .pulsaaxis-transfer\n` +
  	`3. Pulsa Mix - Ketik: .pulsaaxis-mix\n\n` +
  	`Silakan pilih salah satu layanan di atas dengan mengetik perintah yang sesuai.`;
  ...
  case "menupulsasmartfren":
  {
  	...
  	`≡ *Saldo* : ${formatmoney(slo)}\n` +
  	`≡ *Role* : ${cek("role", m.sender)}\n\n` +
  	`*Berikut adalah daftar Pulsa Smartfren yang tersedia:*\n\n` +
  	`1. Pulsa Normal - Ketik: .pulsasmr\n` +
  	`2. Pulsa Transfer - Ketik: .smartfren-transfer\n\n` +
  	`Silakan pilih salah satu layanan di atas dengan mengetik perintah yang sesuai.`;
  ...
  case "menupulsatri":
  {
  	...
  	`≡ *Saldo* : ${formatmoney(slo)}\n` +
  	`≡ *Role* : ${cek("role", m.sender)}\n\n` +
  	`*Berikut adalah daftar Pulsa Tri yang tersedia:*\n\n` +
  	`1. Pulsa Normal - Ketik: .pulsatri\n` +
  	`2. Pulsa Transfer - Ketik: .pulsatri-transfer\n` +
  	`3. Pulsa Mix - Ketik: .pulsatri-mix\n\n` +
  	`Silakan pilih salah satu layanan di atas dengan mengetik perintah yang sesuai.`;
  ...
  case "menupulsaxl":
  {
  	...
  	`≡ *Saldo* : ${formatmoney(slo)}\n` +
  	`≡ *Role* : ${cek("role", m.sender)}\n\n` +
  	`*Berikut adalah daftar Pulsa XL yang tersedia:*\n\n` +
  	`1. Pulsa Normal - Ketik: .pulsaxl\n` +
  	`2. Pulsa Transfer - Ketik: .pulsaxl-transfer\n` +
  	`3. Pulsa Mix - Ketik: .pulsaxl-mix\n\n` +
  	`Silakan pilih salah satu layanan di atas dengan mengetik perintah yang sesuai.`;
  ...
  case "menupulsabyu":
  {
  	...
  	`≡ *Saldo* : ${formatmoney(slo)}\n` +
  	`≡ *Role* : ${cek("role", m.sender)}\n\n` +
  	`*Berikut adalah daftar Pulsa By.U yang tersedia:*\n\n` +
  	`1. Pulsa Normal - Ketik: .pulsabyu\n` +
  	`2. Pulsa Transfer - Ketik: .pulsabyu-transfer\n\n` +
  	`Silakan pilih salah satu layanan di atas dengan mengetik perintah yang sesuai.`;
  ...
  case "menukuotaaxis":
  {
  	...
  	`≡ *Saldo* : ${formatmoney(slo)}\n` +
  	`≡ *Role* : ${cek("role", m.sender)}\n\n` +
  	`*Berikut adalah daftar kuota Axis yang tersedia:*\n\n` +
  	`1. Kuota Normal - Ketik: .umumaxis\n` +
  	`2. Kuota Mini - Ketik: .miniaxis\n` +
  	`3. Kuota Kzl - Ketik: .kzlaxis\n` +
  	`4. Kuota Bronet - Ketik: .bronetaxis\n` +
  	`5. Kuota Warnet - Ketik: .warnetaxis\n` +
  	`6. Kuota Ekstra - Ketik: .ekstraaxis\n` +
  	`7. Kuota Owsem - Ketik: .owsemaxis\n` +
  	`8. Kuota Games - Ketik: .gameaxis\n\n` +
  	`Silakan pilih salah satu layanan di atas dengan mengetik perintah yang sesuai.`;
  ...
  case "menukuotaindosat":
  {
  	...
  	`≡ *Saldo* : ${formatmoney(slo)}\n` +
  	`≡ *Role* : ${cek("role", m.sender)}\n\n` +
  	`*Berikut adalah daftar kuota Indosat yang tersedia:*\n\n` +
  	`1. Kuota Normal - Ketik: .paketind\n` +
  	`2. Kuota Yellow - Ketik: .yellow-ind\n` +
  	`3. Kuota Yellow Gift - Ketik: .yellow-gift\n` +
  	`4. Kuota Gift Data - Ketik: .giftdata\n` +
  	`5. Freedom Harian - Ketik: .freedom-harian\n` +
  	`6. Freedom Internet - Ketik: .freedom-internet\n` +
  	`7. Freedom Internet Gift - Ketik: .freedom-internet-gift\n` +
  	`8. Freedom Internet U - Ketik: .freedom-u\n` +
  	`9. Freedom U Gift - Ketik: .freedom-u-gift\n` +
  	`10. Kuota UMKM - Ketik: .ind-umkm\n\n` +
  	`Silakan pilih salah satu layanan di atas dengan mengetik perintah yang sesuai.`;
...
case "menukuotasmartfren":
{
	...
	`≡ *Saldo* : ${formatmoney(slo)}\n` +
	`≡ *Role* : ${cek("role", m.sender)}\n\n` +
	`*Berikut adalah daftar kuota Smartfren yang tersedia:*\n\n` +
	`1. Kuota Normal - Ketik: .paketsmr\n` +
	`2. Kuota Unlimited - Ketik: .smartfren-unlimited\n` +
	`3. Kuota Volume - Ketik: .smartfren-volume\n` +
	`4. Kuota Youtube - Ketik: .smartfren-youtube\n` +
	`5. Kuota Connex Evo - Ketik: .smartfren-connex\n` +
	`6. Kuota Gokil Max - Ketik: .smartfren-gokil-max\n` +
	`7. Kuota Gokil Combo - Ketik: .smartfren-gokil-combo\n` +
	`8. Kuota Nonstop - Ketik: .smartfren-1on+\n` +
	`9. Kuota Unlimited Nonstop - Ketik: .smartfren-unlimited-nonstop\n` +
	`10. Kuota (Kuota) - Ketik: .smartfren-kuota\n` +
	`11. Kuota Mandiri - Ketik: .smartfren-mandiri\n\n` +
	`Silakan pilih salah satu layanan di atas dengan mengetik perintah yang sesuai.`;
...
break;
  ...
  case "kuota":
  {
  	var slo = cek("saldo", m.sender);
  	if (cek("id", m.sender) == null)
  		return reply(`_Anda Belum Terdaftar di Database. Silahkan ketik *#daftar*_`);
  	...
  	`≡ *Saldo* : ${formatmoney(slo)}\n` +
  	`≡ *Role* : ${cek("role", m.sender)}\n\n` +
  	`*Berikut adalah daftar operator kuota internet yang tersedia:*\n\n` +
  	`1. Kuota Axis - Ketik: .menukuotaaxis\n` +
  	`2. Paket Telkomsel - Ketik: .paketssel\n` +
  	`3. Paket Indosat - Ketik: .menukuotaindosat\n` +
  	`4. Paket Smartfren - Ketik: .menukuotasmartfren\n` +
  	`5. Paket XL - Ketik: .paketxl\n` +
  	`6. Paket Tri - Ketik: .pakettri\n` +
  	`7. Paket By.U - Ketik: .paketbyu\n\n` +
  	`Silakan pilih salah satu operator di atas dengan mengetik perintah yang sesuai.`;
  ...
  case "depopay": {
    ...
    try {
        ...
        if (responseData.success) {
        	...
        	`> _›› Fee:_ ${formatmoney(data.fee)}\n` +
        	`> _›› Saldo Di Terima:_ ${data.balance}\n` +
        	`> _›› Total:_ ${formatmoney(data.amount)}\n` +
        	`> _›› Expired:_ ${data.expired}\n\n` +
        	`Silakan Buka Url dan scan QRIS tersebut sebelum waktu expired 5 menit. Jika sudah melakukan pembayaran, harap tunggu verifikasi.`;
...
case "cancel":
{
	...
	let data_depo = JSON.parse(
		fs.readFileSync(PathAutoo + sender.split("@")[0] + ".json")
      	...
      	axios(config)
      	.then(function (response) {
      		reply(` Berhasil Di Batalkan Deposit Dengan ID: ${data_depo.ref}`);
      		fs.unlinkSync(PathAutoo + m.sender.split("@")[0] + ".json");
      	})
      	.catch(function (error) {
      		reply(`Error`);
